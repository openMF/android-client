...
import com.mifos.api.mappers.GenericMapper.gson
...
import com.mifos.api.mappers.accounts.savings.GetSavingsAccountsAccountIdResponseMapper
...
class DataManagerSavings @Inject constructor(
    ...
    private val SAVINGS_COMMAND_ACTIVATE = "activate"
    private val SAVINGS_COMMAND_APPROVE = "approve"

    private val savingsAccountApi: SavingsAccountApi
        private get() = sdkBaseApiManager.getSavingsApi()
    private val savingsProductApi: SavingsProductApi
        private get() = sdkBaseApiManager.getClient().savingsProducts
    private val savingsChargesApi: SavingsChargesApi
        private get() = sdkBaseApiManager.getClient().savingsAccountCharges
    private val savingsTransactionApi: SavingsAccountTransactionsApi
        private get() = sdkBaseApiManager.getClient().savingsTransactions
    ...
    fun getSavingsAccount(
        ...
        return when (PrefManager.getUserStatus()) {
            0 -> {
                savingsAccountApi
                    .retrieveOne24(savingsAccountId.toLong(), null, null)
                    .map(GetSavingsAccountsAccountIdResponseMapper::mapFromEntity)
    ...
    fun activateSavings(
        ...
        val req: PostSavingsAccountsAccountIdRequest  = convert(request)
        return savingsAccountApi.handleCommands6(
            savingsAccountId.toLong(),
            req, SAVINGS_COMMAND_ACTIVATE
        )
            .map {
                val response = GenericResponse()
                val map = HashMap<String, Any?>()
                map["resourceId"] = it.resourceId
                map["officeId"] = it.officeId
                map["clientId"] = it.clientId
    ...
    fun getSavingsAccountTransactionTemplate(
        ...
        return when (PrefManager.getUserStatus()) {
            0 -> savingsTransactionApi.retrieveTemplate18(savingsAccountId.toLong()).map {
                    convert<SavingsAccountTransactionTemplate>(it)
                }
            1 ->
                /**
    ...
    fun syncSavingsAccountTransactionTemplate(
        ...
        return savingsTransactionApi.retrieveTemplate18(savingsAccountId.toLong()).map {
            convert<SavingsAccountTransactionTemplate>(it)
    ...
    fun processTransaction(
        ...
        return when (PrefManager.getUserStatus()) {
            0 -> savingsTransactionApi.transaction2(savingsAccountId.toLong(), transactionType, req)
                .map { convert<SavingsAccountTransactionResponse>(it) }
            1 ->
                /**
    ...
    fun savingsAccounts(): Observable<List<ProductSavings>> {
        return savingsProductApi.retrieveAll34().map{
            val res = ArrayList<ProductSavings>();
            it.forEach { savingsProduct ->
                res.add(convert(savingsProduct))
            }
            res
        }
        //return mBaseApiManager.savingsApi.allSavingsAccounts
    }

    fun createSavingsAccount(savingsPayload: SavingsPayload?): Observable<Savings> {
        // todo: #1, response mismatch PostSavingsAccountsResponse and Savings
        // 1.5.0 Fineract version works fine with the older mBaseApiManager
        /*val req: PostSavingsAccountsRequest = convert(savingsPayload)
        savingsAccountApi.submitApplication2(req)*/
    ...
    fun savingsAccountTemplate(): Observable<SavingProductsTemplate> {
        return savingsProductApi.retrieveTemplate19()
            .map(SavingsProductTemplateMapper::mapFromEntity)
    }
    ...
    fun getClientSavingsAccountTemplateByProduct(
        ...
        // todo: #2, invalid sdk class definition GetSavingsAccountsTemplateResponse
        // 1.5.0 Fineract version works fine with the older mBaseApiManager
        /*return savingsAccountApi.template14(clientId.toLong(), null, productId.toLong(), null).map{
            convert<GetSavingsAccountsTemplateResponse, SavingProductsTemplate>(it)
        }*/
    ...
    fun getGroupSavingsAccountTemplateByProduct(
        ...
        // todo: #3, invalid sdk class definition GetSavingsAccountsTemplateResponse
        // 1.5.0 Fineract version works fine with the older mBaseApiManager
        /*return savingsAccountApi.template14(null, groupId.toLong(), productId.toLong(), null).map{
            convert<GetSavingsAccountsTemplateResponse, SavingProductsTemplate>(it)
        }*/
    ...
    fun approveSavingsApplication(
        ...
        val request: PostSavingsAccountsAccountIdRequest  = convert(savingsApproval)
        return savingsAccountApi.handleCommands6(
            savingsAccountId.toLong(),
            request, SAVINGS_COMMAND_APPROVE
        )
            .map {
                val response = GenericResponse()
                val map = HashMap<String, Any?>()
        ...
        //return mBaseApiManager.getSavingsApi().approveSavingsApplication(savingsAccountId, savingsApproval);
    }
}