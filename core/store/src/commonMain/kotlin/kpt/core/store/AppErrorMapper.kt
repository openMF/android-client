...
/**
 * Application-level error → user-facing message mapper — mirrors the template's
 * `kpt.core.store.AppErrorMapper` shape verbatim (offline-first-template-migration
 * 02-store-infra-screenstate T3), kept at the template's own `kpt.core.store` package (infra/framework-level seam — kpt is the fixed base namespace).
 *
 * Two flavours:
 ...
 *
 * `core/store/build.gradle.kts`'s `compose.resources { packageOfResClass = }` declares
 * `kpt.core.store.generated.resources`, matching this file's own `kpt.core.store` package.
 */
