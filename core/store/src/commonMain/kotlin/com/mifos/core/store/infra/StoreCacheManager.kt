 ...
 */
package com.mifos.core.store.infra

/**
 * Manages Store cache lifecycle.
 *
 * - Call [clearAll] on logout to prevent stale data leaking across user sessions
 *   (field officers may share a device between visits).
 * - Call [pruneExpiredDrafts] on app start to clean up old SUBMITTED/FAILED draft
 *   rows. PENDING drafts are never pruned — the user may still want to resume them.
 */
